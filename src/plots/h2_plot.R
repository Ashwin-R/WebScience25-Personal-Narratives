...
plot_df <- data.frame(
  is_narrative = c(FALSE, TRUE, FALSE, TRUE),
  quartile_type = c("Least active", "Least active", "Newcomers", "Newcomers"),
  prob = c(0.0143, 0.0185, 0.0451, 0.0518),
  asymp.LCL = c(0.0116, 0.0146, 0.0368, 0.0419),
  asymp.UCL = c(0.0177, 0.0233, 0.0552, 0.0638)
)

# Convert is_narrative to a factor with descriptive labels
plot_df <- plot_df %>%
  mutate(is_narrative = factor(is_narrative, 
                               levels = c(FALSE, TRUE),
                               labels = c("Other comments", 
                                          "Personal narratives")))

# Define colors matching the Set2 palette used in the Python code
# Selecting two colors to match with the Set2 palette
set2_colors <- c("#66c2a5", "#fc8d62", "#8da0cb", "#e78ac3", "#a6d854")
custom_colors <- c("Least active" = set2_colors[2],  # Second color from Set2
                   "Newcomers" = set2_colors[3])     # Third color from Set2

# Create the plot
p <- ggplot(plot_df, aes(x = is_narrative, y = prob, color = quartile_type, group = quartile_type)) +
  ...
  # Add text labels above points for probability values matching H1 style
  ...
  labs(
    x = NULL, 
    y = "Mean probability of replying", 
    color = "User group"
  ) +
...
ggsave("h2.png", plot = p, width = 14, height = 10, units = "in", dpi = 600)